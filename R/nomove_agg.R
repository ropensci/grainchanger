...
#'   included are: \itemize{ \item \code{nm_shei} - Shannon evenness, requires the
#'   additional argument \code{lc_class} (vector or scalar) \item \code{nm_prop} -
...
nomove_agg <- function(g, dat, fun, is_grid = TRUE, ...) {
  checkmate::assert(
  ...
  if ("RasterLayer" %in% class(g)) {
    ...
    fun_args <- list(...)
    out <- raster::aggregate(dat, fact = a, fun = function(x, ...) {
